...
- **Não há build nem servidor.** O "teste" de ponta a ponta continua sendo o Yago rodar e reportar. Há, desde set/2026, **testes unitários** das funções puras de verificação de integridade: `npm test` (node --test, sem dependência externa) roda `src/lib/*.test.js`. Rodar antes de entregar mudança em `integridade.js`/`diagnostico.js`.
...
- **SQLite** (`data/newsletter.db`, via `better-sqlite3`) — banco de dados da implementação Node.js atual (ver `src/lib/db.js`). O arquivo `.db` é commitado no próprio repo pelo bot (`Lets Insights Bot <bot@lets.com.br>`) ao final de cada workflow, mesmo padrão já usado pra imagens geradas (`src/lib/gitAssets.js`) — necessário porque os jobs do GitHub Actions são efêmeros e o próximo job da cadeia (`needs`) precisa enxergar o estado atualizado via checkout do `main`.
  - Tabela **edicoes** (PK `edicao`) — colunas: `edicao, titulo_edicao, pre_header, json_artigos_principais, json_artigos_cards, json_cta, status, gerado_em, enviado_em, json_blog, json_assunto_manchetes`. **jul/2026:** fim da divisão principais/cards (ver "Decisões fechadas" e WF-02/WF-03 abaixo) — `json_artigos_principais` agora guarda os 3 artigos selecionados (sem "cards" menores); `json_artigos_cards` ficou **sem uso** (migração aditiva, coluna não removida pra evitar migração destrutiva); `json_blog` (nova, `ALTER TABLE` em `db.js`) guarda o destaque do blog Lets, escrito pela IA no WF-02 — antes vinha de scraping ao vivo no WF-03, sem passar pela IA e sem persistir; `json_assunto_manchetes` (`ALTER TABLE` em `db.js`, jul/2026) guardava as manchetes do teste de Assunto formato Brazil Journal — **sem uso** desde a reversão de ago/2026 (ver regra 7 abaixo), coluna mantida só pra não exigir migração destrutiva (edições 8-9 continuam legíveis).
    `json_diagnostico` (`ALTER TABLE` em `db.js`, set/2026) guarda o diagnóstico das verificações de integridade daquela edição (descartes e ressalvas), lido pelo WF-03 pra montar o bloco de diagnóstico do preview.
...
8. **ClickUp:** **NUNCA** criar/editar/ler/mover/comentar tasks **sem autorização explícita do Yago na mensagem específica** — mesmo que pareça implícito. Sempre ler uma task antes de atualizar. (Workspace 36916834; tasks de referência: `86ahfmnwx` projeto, `86ahderm5` log de resultados.) **Exceção pontual (ago/2026):** um job automatizado tem permissão de atualizar especificamente a tabela de números do E-goi na task `86ahderm5`, sem precisar de autorização a cada execução — não vale pra mais nada nessa task nem em nenhuma outra (criar/apagar/comentar/mover continuam exigindo autorização explícita).
...
- **Verificar Integridade** (set/2026, `src/lib/integridade.js`): confere se a página ainda é a notícia que o feed anunciou (âncora do resumo, sobreposição de título, host do og:image, idioma/vocabulário, host final após redirect). Roda sobre o HTML já baixado em "Buscar HTML do Artigo" — zero requisição e zero token a mais. Reprovado vira quarentena (cai pro backup, motivo vai pro `json_diagnostico`), nunca retry de IA. Ver ponto de atenção 10.
...
- **Reconferir links** (set/2026, `revalidarLinks`): rebaixa os 4 links (blog + 3 artigos) e roda a mesma verificação de integridade do WF-02, usando o `titulo_original` gravado no banco. Motivo grave ⇒ preview com assunto `[BLOQUEADO]` e alerta no topo, rascunhos do E-goi **não** criados, status preservado, job falha. Fecha a janela entre gerar (terça de madrugada) e disparar (manual, dias depois). Ver ponto de atenção 10.
...
### WF-04 (sexta, 3 dias depois) — Relatório ClickUp
```
Toda Sexta 09:00 BRT → Coletar campanhas Newsletter no E-goi → Agrupar por
data/lista → Calcular Abertura/CTR/Botão → Ler task ClickUp → Substituir
tabela → Escrever task ClickUp → Verificar integridade
```
- **Workflow separado** (`.github/workflows/report-clickup.yml`, não encadeado com WF-01/02/03) rodando `src/report-clickup.js`. Atualiza a tabela "Números Baseados na campanha do Egoi" na task ClickUp `86ahderm5` (workspace `36916834`) — ver regra 8 (exceção pontual de ClickUp).
- **Numeração:** a linha "Edição 1" (piloto, 28/05/2026, "Les's Insights 1...") fica hardcoded no script (`LINHAS_PILOTO`), nunca recalculada. As edições da série "Newsletter" são numeradas por **ordem cronológica de envio** (agrupando campanhas por dia), começando em 2 — não pelo número no `internal_name` do E-goi (que tem bugs de duplicação/desalinhamento, ver correção de numeração 2026-08-10 na memória do projeto). Isso resolve os bugs automaticamente pra sempre, inclusive edições futuras, sem exceções hardcoded por data.
- **Métricas:** Abertura = `unique_opens ÷ sends`; CTR = `unique_clicks ÷ unique_opens`; Botão = cliques na URL contendo `solicitar-proposta` ÷ `unique_opens` (ver `lib/egoi-reports.js`, que agora sempre pede `url=true` no relatório).
- **Risco conhecido e mitigação:** a API do ClickUp tem um bug reportado (buscar `markdown_description` e reenviar via `markdown_content` às vezes corrompe a formatação). Por decisão do Yago (2026-08-10), o script sobrescreve a descrição inteira mesmo assim — mas faz um **regex replace cirúrgico** (só o bloco da tabela, preservando o resto) e uma **verificação pós-escrita** (confere que marcadores-chave como `## Sobre o projeto` e `_Responsável: Yago_` sobreviveram; se sumirem, falha o job ruidosamente em vez de deixar a corrupção passar despercebida).
- Requer secrets `EGOI_API_KEY` e `CLICKUP_API_TOKEN` (token pessoal, gerado em ClickUp → Configurações → Apps → API Token) no GitHub Actions.
...
10. **URL sequestrada por parasite SEO (set/2026, incidente real, protegido):** a edição 16 saiu com um card apontando pra `ocarreteiro.com.br/empresas/veic-redes/`, URL que passou a servir página de cassino indonésio ("ULTRA777 Daftar Situs Slot Toto"). O feed RSS do O Carreteiro continuava anunciando o título e o resumo legítimos, o resto do portal estava normal, e a URL respondia **HTTP 200** — então `validarURL` (que só olha status) aprovou. O único sinal que o pipeline chegou a ver foi o og:image apontando pra `i.pinimg.com` (Pinterest), que ninguém conferia. Lição central: **disponibilidade não é integridade**; a verificação tem que provar que a página ainda é a notícia que o feed prometeu.
    **Proteção implementada (`src/lib/integridade.js`, com testes em `integridade.test.js`):** todas as checagens rodam sobre o HTML que o WF-02 já baixava pra extrair a imagem e descartava em seguida, então **custo zero de requisição e de token**. São cinco: (1) **âncora do resumo** — alguma janela de 8 palavras do resumo do RSS tem que aparecer no corpo da página; (2) **sobreposição de título** — o `<title>` precisa reter ao menos 30% dos tokens significativos do título do RSS (não basta exigir sobreposição zero: o template de spam ecoava "Veic Redes" do slug); (3) **host do og:image** — lista de hosts de UGC (pinimg, imgur, blogspot...) como sinal forte, host meramente diferente como sinal fraco (portais legítimos usam CDN de terceiro, ex: Brasil Mineral via `storage.googleapis.com`); (4) **idioma e vocabulário** — piso de marcadores de português no texto e lista de expressões inequívocas de spam (fora da lista de propósito: "aposta", "slot" e "bet" isolados, que aparecem em matéria legítima de logística); (5) **host final após redirects**. Pesos: um motivo `grave` reprova sozinho, dois `leve` somados reprovam, um `leve` isolado passa e vira ressalva.
    **Quarentena, nunca retry:** artigo reprovado sai da lista e o próximo backup ocupa o lugar. **Nenhuma reprovação dispara nova chamada de curadoria ou redação** — é o único jeito de esse mecanismo custar tokens, e por isso não existe.
    **Reconferência no WF-03 (`revalidarLinks`):** o disparo real é manual e sai dias depois da geração, então os 4 links (blog + 3 artigos) são reconferidos imediatamente antes de montar o HTML. Se algum tiver motivo grave: o preview sai com assunto `[BLOQUEADO]` e o alerta no topo, **os rascunhos do E-goi não são criados**, o status da edição **não** é alterado (basta rodar o WF-03 de novo depois de corrigir) e o job falha ruidosamente. Lá a comparação usa `titulo_original` (título como o feed publicou), agora gravado junto de cada artigo em `json_artigos_principais`.
    **Diagnóstico no preview (`src/lib/diagnostico.js`):** descartes e ressalvas ficam registrados em `json_diagnostico` (coluna aditiva em `edicoes`) e aparecem num bloco no topo do **corpo do preview**. Nunca no HTML de produção, nem nos anexos, nem no rascunho do E-goi. Antes disso, um descarte só existia no log do Actions.
    **O que essas camadas não pegam:** página sequestrada que preserve título e resumo e injete só links maliciosos no corpo. Para esse caso seria preciso um juiz de IA (avaliado: ~US$ 0,005/edição em Haiku), deixado de fora por enquanto — ver "Otimização de custo de tokens".
