...
7. **Assunto:** manchete única primeiro, marca no fim: `[manchete] « Let's Insights`. A IA gera **só a manchete**, em `titulo_edicao` (máx 45 chars, ideal 30-42, começando pelo fato concreto, sem o nome da newsletter em lugar nenhum); o sufixo ` « Let's Insights` é acrescentado **em código** por `montarAssunto()` (`src/lib/template.js`), que também normaliza defensivamente se a IA reincidir no formato antigo ou já devolver o sufixo. `titulo_edicao` segue limpo (sem marca) no `<title>` do HTML e no `internal_name` do E-goi. **set/2026 — por que a marca saiu da frente:** medição nas 15 edições enviadas mostrou que o prefixo `Let's Insights · ` consumia 17 dos ~33 caracteres que o Gmail Android exibe (52%) e que **0 de 15** assuntos chegavam inteiros ao leitor de celular — todos cortavam no meio da palavra. A marca já aparece no campo de remetente em toda caixa de entrada, então repeti-la na frente gastava a metade mais valiosa da vitrine com o elemento que menos converte (decisão do Yago, alinhada com TLDR, que não usa marca nenhuma no assunto, e com Brazil Journal, que a põe no fim). **Histórico:** o formato antigo era `Let's Insights · [destaque]` (ponto médio U+00B7); entre jul-ago/2026 houve um teste com 2 manchetes (`assunto_manchetes`, separadas por `; `, usado nas edições 8 e 9), revertido por dado insuficiente — o layout mudou junto e não deu pra isolar o efeito. **Lição metodológica: ao testar assunto de novo, mudar SÓ o assunto e segurar o resto por 4-6 edições.**
